...
export default function Header() {
  ...
  return (
    <>
          ...
          <nav className="hidden md:flex gap-8 text-sm tracking-widest [font-family:var(--font-imprima)]">
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="relative opacity-60 hover:opacity-100 transition-all duration-300 group"
              >
                {item.name}

                {/* Underline animation */}
                <span className="absolute left-0 -bottom-1 w-0 h-[1px] bg-gradient-to-r from-[#6c6c6c] via-[#ffffff] to-[#000000] transition-all duration-300 ease-out group-hover:w-full"></span>    </Link>
            ))}
          </nav>
      ...
      {/* 🔥 SIDEBAR OVERLAY */}
      <div
        className={`fixed inset-0 z-50 transition ${open ? "visible opacity-100" : "invisible opacity-0"
          }`}
      >
        {/* Background blur */}
        <div
        ...
        {/* Sidebar */}
        <div
          className={`absolute right-0 top-0 h-full w-[70%] max-w-[300px] bg-black border-l border-white/10 p-6 transform transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"
            }`}
        >
          {/* Close */}
          ...
          <nav className="flex flex-col gap-6 text-lg tracking-widest text-white [font-family:var(--font-imprima)]">
            ...
            <div className="pt-6 border-t border-white/10">
              <button onClick={() => router.push("/admin")} className="w-full py-3 text-sm tracking-widest border border-white/20 rounded-md hover:border-white/40 transition-all duration-300 [font-family:var(--font-imprima)]">
                ADMIN PANEL
              </button>
            </div>
          </nav>
